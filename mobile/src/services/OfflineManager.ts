...
import { SyncOperation, SyncOperationType, SyncEntity, SyncStatus } from '../constants/types';
...
export class OfflineManager {
  ...
  private async syncOperation(operation: SyncOperation): Promise<boolean> {
    ...
    try {
      ...
      await this.updateLocalEntitySyncStatus(operation.entity, operation.entityId, 'synced');
      ...
      const newStatus =
        newRetryCount >= config.SYNC.MAX_RETRIES ? SyncStatus.FAILED : SyncStatus.PENDING;
      ...
      if (newStatus === SyncStatus.FAILED) {
        await this.updateLocalEntitySyncStatus(operation.entity, operation.entityId, 'failed');
      }
  ...
  private calculateBackoff(retryCount: number): number {
    ...
    const delayMs = Math.min(baseDelayMs * Math.pow(2, retryCount - 1), maxDelayMs);
    return delayMs;
  }
  ...
  async syncAll(): Promise<void> {
    ...
    try {
      ...
      const operations = await sqliteService.getWhere<SyncOperation>('sync_queue', {
        status: SyncStatus.PENDING,
      });
      ...
      for (let i = 0; i < operations.length; i += batchSize) {
        ...
        await Promise.allSettled(batch.map((operation) => this.syncOperation(operation)));
      }

      // Update last sync time
      await asyncStorage.set(config.STORAGE_KEYS.LAST_SYNC, new Date().toISOString());
